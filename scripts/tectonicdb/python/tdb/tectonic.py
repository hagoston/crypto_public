...
class TectonicDB():
    ...
    async def cmd(self, cmd):
        ...
        if type(cmd) != str:
            ...
            message = (cmd+'\n').encode()
        # insert msg size as big endian 4 byte array
        cmdlen = len(cmd) + 1
        message = struct.pack('>I', cmdlen) + message
    ...
    #
    # async def bulkaddinto(self, updates, dbname):
    #     ret = await self.cmd("BULKADD INTO " + dbname)
    #     print(ret)
    #     for update in updates:
    #         ts, seq, is_trade, is_bid, price, size = update
    #
